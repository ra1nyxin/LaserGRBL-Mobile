...
- 记录跳过的暂不支持命令，例如 `C`、`Q`、`A`

SVG 转换适合简单 logo、线稿、外框和已经转成直线段的矢量图。当前不会把贝塞尔曲线自动展平为线段；复杂 SVG 建议先在矢量软件中转为折线或导出为 G-code。
...
- 已支持基础图片栅格转 G-code。
- SVG 转换当前只支持基础线段 path，不支持自动展平贝塞尔曲线、圆弧和复杂 transform。
