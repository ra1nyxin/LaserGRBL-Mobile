...
- 颜色分层开启时，会按颜色层分组输出，并自动给不同颜色层设置相对功率和进给
- 支持相对坐标和绝对坐标
- 按设定宽度缩放到实际加工尺寸
- 生成 `G0` 空程、`M4 S...` 开光、`G1` 线段雕刻、`M5` 关光
- 记录跳过的暂不支持 path 命令，方便排查复杂 SVG

SVG 转换适合简单 logo、线稿、外框和常见矢量图形。贝塞尔曲线和 SVG path 圆弧会展平成多段 `G1`，圆形和椭圆会近似为多边形路径。颜色分层默认开启，黑色或默认图层使用当前 SVG 参数，红色偏切割，蓝色偏浅标记，绿色为中等标记，黄/橙为轻标记；所有颜色层功率都不会超过用户设置的最大功率。复杂 SVG 仍建议先在矢量软件里检查尺寸、删除隐藏对象，并用边界预跑确认实际范围。
...
转换参数里的按钮、滑块、复选框和说明文字都使用 Material3 主题颜色，不写死亮色或暗色。新增的 SVG 颜色分层开关会跟随当前浅色 / 深色主题切换，控件和文字颜色与其他转换参数保持一致。
...
- 已支持基础图片栅格转 G-code。
- SVG 转换已支持常见 path、基础图形、贝塞尔曲线展平、path 圆弧展平、颜色分层和常见 transform，但不会处理外部 CSS 文件、复杂选择器、填充区域扫描和布尔运算。
