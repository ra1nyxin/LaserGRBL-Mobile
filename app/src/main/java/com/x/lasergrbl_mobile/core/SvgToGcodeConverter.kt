...
object SvgToGcodeConverter {
    ...
    private fun buildOutputLayers(paths: List<ParsedPath>, settings: SvgGcodeSettings): List<SvgOutputLayer> {
        if (!settings.colorLayering) {
            return listOf(
                SvgOutputLayer(
                    profile = SvgLayerProfile("single", settings.power, settings.feedRate),
                    paths = paths,
                )
            )
        }
        ...
        return ordered.map { (layer, layerPaths) ->
            SvgOutputLayer(
                profile = layerProfile(layer, settings),
                paths = layerPaths,
            )
        }
    }
    ...
    private fun layerProfile(layer: SvgLayerKey, settings: SvgGcodeSettings): SvgLayerProfile {
        ...
        if (rgb == null) {
            return SvgLayerProfile(layer.label, settings.power, settings.feedRate)
        }
        ...
        return SvgLayerProfile(layer.label, power, feed)
    }
